...
Use the `form-item` in any custom list - other than Priority Ionic's [`FormList`](./FormList).
...
Similar to `avatar`, setting the `thumbnail` property will display the column's value as a thumbnail in the item.
...
Set the `slidingButtons` property to turn the `form-item` into a sliding item that can be swiped to the left or right to reveal these buttons. `slidingButtons` is an array where each button is a `ButtonOption` object, which defines the `icon`, `color`, `text`, `side` and `click` handler for the button.
...
A CSS class with the same name as the column name is added to the container element of each column displayed. This class is added to both the column's title element and value element with addition of a 'title' and 'value' class respectively. A subform name CSS class is added to the subform's element as well.
